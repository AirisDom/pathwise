    ...
    <StudentLayout>
      <div className="max-w-6xl mx-auto px-4 py-6">
        <div className="mb-4">
          ...
          <p className="text-gray-500 mt-1">Chat with your course creators</p>
        </div>

        <div className="h-[calc(100vh-220px)] flex bg-white border border-gray-200 rounded-xl overflow-hidden">
          {/* Conversation list */}
          <div
            ...
            {loading ? (
              ...
              <div className="flex flex-col items-center justify-center flex-1 p-6 text-center">
                ...
                <p className="text-xs text-gray-400 mt-1">
                  Message a creator from any course page
                </p>
              </div>
            ) : (
                ...
                {conversations.map((convo) => (
                  ...
                  >
                    {convo.otherUser.image ? (
                      <img src={convo.otherUser.image} alt="" className="w-10 h-10 rounded-full object-cover shrink-0" />
                    ) : (
            ...
            {!activeThread ? (
              ...
              <>
                <div className="flex items-center gap-3 px-4 py-3 border-b border-gray-200 bg-gray-50/50">
                  <button onClick={() => setActiveThread(null)} className="md:hidden text-gray-600 hover:text-gray-900">
                    <ArrowLeft className="w-5 h-5" />
                  </button>
                  {activeConvo?.otherUser.image ? (
                    <img src={activeConvo.otherUser.image} alt="" className="w-8 h-8 rounded-full object-cover" />
                  ) : (
                  ...
                  <div>
                    <p className="text-sm font-semibold text-gray-900">{activeConvo?.otherUser.name || "Creator"}</p>
                    <p className="text-xs text-gray-500 capitalize">{activeConvo?.otherUser.role?.toLowerCase()}</p>
                  </div>
                </div>
                    ...
                    messages.map((msg) => {
                      ...
                      return (
                        <div key={msg.id} className={`flex ${isMe ? "justify-end" : "justify-start"}`}>
                          <div className={`max-w-[70%] rounded-2xl px-4 py-2.5 ${isMe ? "bg-blue-600 text-white rounded-br-md" : "bg-gray-100 text-gray-900 rounded-bl-md"}`}>
